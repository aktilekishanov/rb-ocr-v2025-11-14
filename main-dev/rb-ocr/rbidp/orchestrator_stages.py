...
from rbidp.utils.artifacts import (
...
from rbidp.utils.io_utils import (
...
def _mk_run_dirs(runs_root: Path, run_id: str) -> dict[str, Path]:
    ...
    return {"base": base_dir, "input": input_dir, "ocr": ocr_dir, "gpt": gpt_dir, "meta": meta_dir}
...
def _finalize_timing_artifacts(ctx: PipelineContext) -> None:
    ...
    ctx.artifacts["gpt_seconds"] = ctx.timers.totals.get("gpt", 0.0)
...
def stage_ocr(ctx: PipelineContext) -> dict[str, Any] | None:
    with stage_timer(ctx, "ocr"):
        textract_result = ask_tesseract(str(ctx.saved_path), output_dir=str(ctx.ocr_dir), save_json=True)
    if not textract_result.get("success"):
        return fail_and_finalize("OCR_FAILED", str(textract_result.get("error")), ctx)

    try:
        filtered_pages_path = filter_ocr_response(
            textract_result.get("raw_obj", {}), str(ctx.ocr_dir), filename=OCR_PAGES
        )
...
def stage_doc_type_check(ctx: PipelineContext) -> dict[str, Any] | None:
    try:
        with stage_timer(ctx, "gpt"):
            dtc_raw_str = check_single_doc_type(ctx.pages_obj)
        dtc_raw_path = ctx.gpt_dir / GPT_DOC_TYPE_RAW
        with open(dtc_raw_path, "w", encoding="utf-8") as f:
            f.write(dtc_raw_str or "")
        dtc_filtered_path = filter_gpt_generic_response(
            str(dtc_raw_path), str(ctx.gpt_dir), filename=GPT_DOC_TYPE_FILTERED
        )
        ctx.artifacts["gpt_doc_type_check_filtered_path"] = str(dtc_filtered_path)
...
def stage_extract_and_stamp(ctx: PipelineContext) -> dict[str, Any] | None:
    try:
        with stage_timer(ctx, "gpt"):
            gpt_raw = extract_doc_data(ctx.pages_obj)
            gpt_raw_path = ctx.gpt_dir / GPT_EXTRACTOR_RAW
            with open(gpt_raw_path, "w", encoding="utf-8") as f:
                f.write(gpt_raw or "")
            filtered_path = filter_gpt_generic_response(
                str(gpt_raw_path), str(ctx.gpt_dir), filename=GPT_EXTRACTOR_FILTERED
            )
        try:
            os.remove(gpt_raw_path)
        except Exception as e:
            logger.debug("Failed to remove gpt_raw_path: %s", e, exc_info=True)
        ctx.artifacts["gpt_extractor_filtered_path"] = str(filtered_path)
    ...
    try:
        with stage_timer(ctx, "gpt"):
            merged_path = merge_extractor_and_doc_type(
                extractor_filtered_path=ctx.artifacts.get("gpt_extractor_filtered_path", ""),
                doc_type_filtered_path=ctx.artifacts.get("gpt_doc_type_check_filtered_path", ""),
                output_dir=str(ctx.gpt_dir),
                filename=MERGED_FILENAME,
                stamp_check_response_path=ctx.artifacts.get("stamp_check_response_path", ""),
            )
        ctx.artifacts["gpt_merged_path"] = str(merged_path)
        try:
    ...
    try:
        with stage_timer(ctx, "gpt"):
            validation = validate_run(
                ...
                merged_path=str(ctx.artifacts.get("gpt_merged_path", "")),
                output_dir=str(ctx.gpt_dir),
