"""
Deterministic and fuzzy FIO matching logic used by the validator.

Provides normalization, variant generation (FULL/LF/FP/L_I/L_IO), and
matching rules to decide whether document FIO matches application FIO,
with rich diagnostics for troubleshooting borderline cases.
"""
...
def parse_fio(raw: str) -> NameParts:
    """
    Heuristically parse raw FIO into structured name parts.

    - Keeps hyphens.
    - Detects compact initials in a single token (e.g., "и.о.").
    - For two tokens, distinguishes likely LAST FIRST vs FIRST PATRO use cases.
    """
