"""
Stamp detector integration for the RB-OCR pipeline (optional stage).

Wraps an external detector script that marks presence of a stamp and
optionally writes visualization images with bounding boxes.
"""
...
# WORKAROUND: hardcoded detector paths for current deployment; should be
# moved to configuration or environment variables when stabilizing.
...
def _render_pdf_to_vertical_jpg(pdf_path: str) -> str | None:
...
def _run_detector(
    image_path: str, vis_dest_dir: str | None = None, vis_basename: str | None = None
) -> bool | None:
            ...
            try:
                ...
                if os.path.exists(det_src):
                    ...
                    for fname in os.listdir(out_dir):
                        if fname.startswith(det_stem + "_with_boxes") or fname.endswith(
                            "_with_boxes" + det_suffix
                        ):
                            try:
                ...
                print(
                    f"[stamp_check] vis copy {'OK' if copied else 'MISS'}: src_dir={out_dir} -> {vis_dst}"
                )
            except Exception:
                pass
...
def stamp_present_for_source(source_path: str, vis_dest_dir: str | None = None) -> bool | None:
