...
class PipelineContext:
    ...
    # NEW: Cache parsed results (eliminates redundant file reads)
...
def finalize_success(verdict: bool, checks: dict[str, Any] | None, ctx: PipelineContext) -> dict[str, Any]:
    ...
    # Use cached data from context (no file I/O!)
...
def stage_acquire(ctx: PipelineContext) -> dict[str, Any] | None:
    ...
    # FIO is now passed via PipelineContext, no need for metadata.json
...
def stage_ocr(ctx: PipelineContext) -> dict[str, Any] | None:
    ...
    try:
        ...
        # Fix: util_read_json returns {"pages": [...]}, we need the list
...
def stage_doc_type_check(ctx: PipelineContext) -> dict[str, Any] | None:
    try:
        ...
        try:
            ...
            # Remove raw file immediately after filtering
            try:
            ...
            # Cache in context for later use (eliminates file re-read)
...
# stage_merge function deleted - no longer needed (validator uses cached context data)
...
def run_pipeline(
    ...
    ctx = PipelineContext(
        ...
        dirs=dirs,
        # NEW: Populate from external metadata
    ...
    for stage in (
        ...
        stage_extract,
        # stage_merge removed - validator now uses cached context data
