"""
Client for the ForteBank Textract-based OCR endpoint (dev).
"""
...
def call_fortebank_textract(pdf_path: str, ocr_engine: str = "tesseract") -> str:
    ...
    # WORKAROUND: for dev servers (non-SSL / self-signed)
    # SECURITY: this must be hardened for production deployments.
...
def ask_textract(pdf_path: str, output_dir: str = "output", save_json: bool = True) -> dict:
    ...
    is_image = bool(
        (mt and mt.startswith("image/"))
        or os.path.splitext(pdf_path)[1].lower()
    ...
    if not success:
        error = (
            (obj.get("message") or obj.get("error") or parse_err or "Unknown OCR error")
            if isinstance(obj, dict)
            else (parse_err or "Unknown OCR error")
        )
    result = {
        "success": success,
        "error": error,
